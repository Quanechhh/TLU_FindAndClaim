...
public class LoginViewModel extends ViewModel {
    ...
    void loginEmail(User user) {
        GlobalData.firebaseAuth.signInWithEmailAndPassword(user.getEmail(), user.getPassword()).addOnSuccessListener(authResult -> {
            if (authResult.getUser() != null) {
                GlobalData.firebaseDB.collection("users").document(authResult.getUser().getUid()).get().addOnSuccessListener(documentSnapshot -> {
    ...
    void changePassword(User user) {
        GlobalData.firebaseAuth.getCurrentUser().updatePassword(user.getPassword())
